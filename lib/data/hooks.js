...
export function useStats(studentId = null) {
  const [stats, setStats] = React.useState(null);
  const [loading, setLoading] = React.useState(true);
  React.useEffect(() => {
    let on = true;
    fetch(withStudent('/api/stats', studentId))
      .then((r) => r.json())
      .then((j) => { if (on) { setStats(j?.success ? j.data : null); setLoading(false); } })
...
export function useSessions(limit = 50, studentId = null) {
  ...
  React.useEffect(() => {
    let on = true;
    fetch(withStudent(`/api/sessions?limit=${limit}`, studentId))
      .then((r) => r.json())
