...
// routes: the user signs in, then pastes the callback link (ChatGPT) or code
// (Grok). connect() opens the provider's page and stays pending until the user
// submits the code or cancels — mirroring the desktop loopback's semantics so
// the UI works the same either way.
const pendingWebConnect = {};
...
export async function aiConnect(uiProvider) {
  ...
  // Stay pending until the user pastes the callback/code (submitCode) or cancels.
  return new Promise((resolve) => { pendingWebConnect[p] = resolve; });
}
...
export async function aiSubmitCode(uiProvider, code) {
  ...
  let j;
  try {
    j = await postJson(`/api/ai/${p}/complete`, { callback: code });
  } catch {
    return { ok: false, error: 'Could not reach the server. Try again.' };
  }
  if (j?.ok) {
    pendingWebConnect[p]?.({ ok: true });
    delete pendingWebConnect[p];
    return { ok: true };
  }
  return { ok: false, error: j?.error || 'That did not work.' };
}
...
export async function aiCancelConnect(uiProvider) {
  ...
  pendingWebConnect[p]?.({ ok: false, error: 'Connection was cancelled.' });
  delete pendingWebConnect[p];
  return { ok: true };
}
