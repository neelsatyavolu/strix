...
function App() {
  ...
  const I = (n) => React.createElement('i', { 'data-lucide': n, style: { width:16, height:16 } });

  React.useEffect(() => { window.lucide && window.lucide.createIcons(); }, [view, tutorOn, dark, role]);
  ...
  const sidebar = (
    <Sidebar
      ...
      footer={<button onClick={() => go('settings')} style={{display:'flex', gap:8, alignItems:'center', width:'100%', padding:'6px 8px', borderRadius:'var(--radius-md)', background:'transparent', border:0, cursor:'pointer', textAlign:'left'}}>
        ...
        <i data-lucide="settings-2" style={{width:14, height:14, color:'var(--text-tertiary)'}}/>
      </button>}
    />
  );
